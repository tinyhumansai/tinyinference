...
impl std::fmt::Display for ProviderError {
    ...
    /// [`crate::Error::Model`] string. Preserving this as a
...
impl StreamAccumulator {
    ...
    pub fn push(&mut self, item: &ModelStreamItem) {
        match item {
            ...
            ModelStreamItem::ProviderFailed(error) => {
                ...
                // `finish()` can surface `Error::Provider` and the retry
    ...
    /// # Errors
    ///
    /// Returns [`crate::Error::Provider`] when a
    ...
    /// as transient — or [`crate::Error::Model`] when an
    ...
    pub fn finish(self) -> Result<ModelResponse> {
        if let Some(error) = self.failed_provider {
            return Err(crate::Error::Provider(Box::new(error)));
        }
