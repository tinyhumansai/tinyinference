...
fn parse_error_body_classifies_retryability_by_http_status() {
    ...
    // into `Error::Model(String)`, so it could not distinguish a
...
fn provider_failed_stream_item_finishes_as_provider_error() {
    // A streamed `ProviderFailed` must finish as `Error::Provider` with
...
async fn sse_stream_parses_text_tool_calls_and_usage() {
    ...
    let bytes = futures::stream::iter(
        raw.into_iter()
            .map(|v| Ok::<bytes::Bytes, Error>(bytes::Bytes::from(v))),
    );
...
fn from_env_errors_when_api_key_missing() {
    let result = OpenAiModel::api_key_from_env("TINYINFERENCE_TEST_KEY_THAT_MUST_NOT_EXIST");
