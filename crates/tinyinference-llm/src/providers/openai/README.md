# tinyinference_llm::providers::openai
...
`tinyinference_llm::Error::Provider`, so
`tinyinference_llm::failure::classify_provider_error` can classify
...
structure to preserve and surface as a plain `tinyinference_llm::Error::Model` string
via `provider_failure_message`. Malformed JSON bodies surface as
`tinyinference_llm::Error::Serialization`.
